...
_DORSEY_TOOL_SCHEMA = build_tool_schema(
    DorseyMoatOutput,
...
class DorseyMoatSkill(SkillBase):
    ...
    def _build_user_message(
        ...
        parts.append(
            ...
            f"```json\n{ratios_json}\n```\n\n"
            "Retourne l'analyse structurée via l'outil dorsey_moat_output."
        )
    ...
    async def execute(
        ...
        """
        Appelle l'API Claude avec Tool Use — le modèle popule dorsey_moat_output directement,
        ...
        response = await call_claude_with_retry(
            ...
            max_tokens=4096,
            tools=[{"name": "dorsey_moat_output", "input_schema": _DORSEY_TOOL_SCHEMA}],
            tool_choice={"type": "tool", "name": "dorsey_moat_output"},
        )
