...
from .schemas import KlarmanInput, KlarmanOutput

logger = logging.getLogger(__name__)

# Schéma dérivé de Pydantic — champs post-assignés exclus.
_KLARMAN_TOOL_SCHEMA = build_tool_schema(
    KlarmanOutput,
    exclude={"citations", "cost_usd"},
)


class KlarmanMarginSkill(SkillBase):
    """
    Skill Tier 2 : marge de sécurité et préservation du capital (Klarman / Baupost).
    Applique Margin of Safety (Klarman, 1991) — classification situation, scoring marge,
    preservation du capital, discount to intrinsic.
    Verdict OPPORTUNITE_FORTE / OPPORTUNITE_MODEREE / ATTENDRE / PASSER.
    """

    skill_id: ClassVar[str] = "klarman_margin"
    tier: ClassVar[int] = 2
    description: ClassVar[str] = (
        "Applique le cadre de Seth Klarman (Baupost Group, +20% CAGR sur 35 ans) — "
        "marge de sécurité absolue, préservation du capital, situations spéciales, distressed. "
        "Verdict OPPORTUNITE_FORTE / OPPORTUNITE_MODEREE / ATTENDRE / PASSER."
    )
    ...
    def _build_user_message(
        self,
        input_data: KlarmanInput,
        citations: list[Citation],
    ) -> str:
        """Construit le message utilisateur avec les ratios Klarman et le type de situation."""
        parts: list[str] = []
        r = input_data.klarman_ratios

        parts.append(
            f"## Analyse Klarman — {input_data.ticker}\n\n"
            f"**Type de situation (indice)** : {input_data.situation_type}\n\n"
            f"### Ratios financiers\n"
            f"- Prix actuel : {r.price}\n"
            f"- NAV par action : {r.nav_per_share}\n"
            f"- Valeur de liquidation : {r.liquidation_value}\n"
            f"- Dette/Capitaux propres : {r.debt_equity}\n"
            f"- Revenus (milliards) : {r.revenue_bn}\n"
            f"- Catalyseur identifié : {r.catalyst}\n"
        )

        if r.nav_per_share is not None and r.nav_per_share > 0:
            discount = (r.nav_per_share - r.price) / r.nav_per_share
            parts.append(f"\n**Discount vs NAV** : {discount:.1%}\n")
        elif r.liquidation_value is not None and r.liquidation_value > 0:
            discount = (r.liquidation_value - r.price) / r.liquidation_value
            parts.append(f"\n**Discount vs liquidation** : {discount:.1%}\n")

        if citations:
            parts.append("\n## Contexte de référence (corpus Klarman marge de sécurité)\n")
        ...
        parts.append(
            f"\nQualifie le type de situation, score la marge de sécurité et la préservation du capital "
            f"pour **{input_data.ticker}** selon le cadre Klarman.\n\n"
            f"Données complètes :\n```json\n{input_json}\n```\n\n"
            "Retourne l'analyse structurée via l'outil klarman_output."
        )

        return "\n".join(parts)

    async def execute(
        self, input_data: KlarmanInput
    ) -> tuple[KlarmanOutput, UsageDetail]:
        """
        Appelle l'API Claude avec Tool Use — le modèle popule klarman_output directement,
        éliminant les hallucinations de format JSON texte.
        """
        rag_query = (
            f"Klarman marge sécurité {input_data.ticker} "
            f"Baupost preservation capital distressed situations spéciales "
            f"absolute return discount intrinsic value"
        )
        ...
        response = await call_claude_with_retry(
            ...
            max_tokens=4096,
            tools=[{"name": "klarman_output", "input_schema": _KLARMAN_TOOL_SCHEMA}],
            tool_choice={"type": "tool", "name": "klarman_output"},
        )
        ...
        output = KlarmanOutput.model_validate(data)
