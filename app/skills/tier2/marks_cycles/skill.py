...
class MarksCyclesSkill(SkillBase):
    """
    Skill Tier 2 : cadre Howard Marks â€” cycles et risque.
    Positionne le pendule du sentiment de marchÃ©,
    applique le second-level thinking, et recommande un timing d'allocation.
    Note : MarksOutput n'a pas de ticker â€” analyse du marchÃ©, pas d'un titre spÃ©cifique.
    """

    skill_id: ClassVar[str] = "marks_cycles_risk"
    tier: ClassVar[int] = 2
    description: ClassVar[str] = (
        "Applique le cadre de Howard Marks (Oaktree Capital) â€” pendule du sentiment de marchÃ©, "
    ...
    def _build_user_message(
        ...
        """Construit le message utilisateur avec le contexte marchÃ© et les indicateurs."""
        parts: list[str] = []
        r = input_data.marks_ratios

        parts.append(
            f"## Contexte de marchÃ©\n{input_data.market_context}\n\n"
            f"### Indicateurs quantitatifs\n"
            f"- P/E marchÃ© : {r.pe_market if r.pe_market is not None else 'Non fourni'}\n"
            f"- VIX (indice de volatilitÃ©) : {r.vix if r.vix is not None else 'Non fourni'}\n"
            f"- Credit spreads : {r.credit_spreads_bps} bps\n"
            if r.credit_spreads_bps is not None
            else f"- Credit spreads : Non fournis\n"
        )

        parts.append(
            f"- Achats nets d'insiders : {r.insider_net_buying}\n"
            if r.insider_net_buying is not None
            else "- Achats nets d'insiders : Non fournis\n"
        )
        parts.append(
            f"- Sentiment haussier (AAII/surveys) : {r.bullish_sentiment_pct * 100:.1f} %\n"
            if r.bullish_sentiment_pct is not None
            else "- Sentiment haussier : Non fourni\n"
        )

        if citations:
            parts.append("\n## Contexte de rÃ©fÃ©rence (corpus Marks)\n")
        ...
        parts.append(
            f"\nApplique le cadre Howard Marks sur ce contexte de marchÃ©. "
            f"Positionne le pendule, gÃ©nÃ¨re un insight second-level, et recommande un timing d'allocation.\n\n"
    ...
    async def execute(
        ...
        """
        Appelle l'API Claude avec prompt caching et retourne (MarksOutput, UsageDetail).
        """
        rag_query = (
