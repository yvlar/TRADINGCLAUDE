...
### Sprint 188 — Refactor : helper `for_each_tenant()` (squelette énumère-et-scope, 5 copies) ✅

**Objectif :** Éliminer la 5ᵉ copie d'un squelette **load-bearing** (finding d'altitude différé des revues S185/S186). Le patron « énumère les tenants (`SELECT id FROM tenants ORDER BY created_at`) → exécute un corps sous `tenant_scope(tenant_id)` → best-effort log-and-continue » était dupliqué **5×** dans `app/workers/tasks.py` — seuil où une divergence silencieuse (un chemin qui oublie le `tenant_scope` → lecture sans filtrage RLS, ou qui avorte au premier échec au lieu de continuer) devient probable. Sprint **backend/worker seul** (aucun fichier `frontend/` touché → non-régression front par construction).

**Réconciliation carte↔code (anti-hallucination) :** la carte mis-identifiait les 5 fonctions hôtes — elle listait `_execute_usage_reporting` (qui énumère en réalité `SELECT … FROM subscriptions ORDER BY tenant_id`, une requête/filtre structurellement différents) et **omettait** `_execute_watchlist_analysis` (ligne 251, vrai 5ᵉ copie). Le `grep -c` = 5 (prémisse cœur) était **vrai** ; seule l'identité d'un hôte était fausse. La vérité terrain (grep) prime → ensemble corrigé : `_execute_watchlist_analysis` / `_execute_price_alert_check` / `_execute_composite_alert_check` / `_execute_scheduled_screener` / `_execute_retention_purge`. `_execute_usage_reporting` (énumère `subscriptions`) et `_execute_analysis` (scope **mono-tenant** passé via Celery, pas une énumération) **correctement non touchés**.

**Décision tranchée — home `app/workers/tenant_iteration.py` (nouveau module) plutôt que co-localisation dans `tasks.py` :** `tasks.py` fait déjà ~1070 lignes ; un module dédié évite de l'alourdir, isole le helper pour un test unitaire propre, et fait tomber la preuve grep à **0** dans `tasks.py` (le helper détient l'unique `SELECT`). L'itération multi-tenant est un patron **worker** (énumérer tous les tenants pour un job planifié) → `app/workers/` est le bon paquet (vs `app/db/` qui héberge les primitives de connexion comme `create_runtime_pool` S187).

**Décision tranchée — le helper RETOURNE `list[T]`, l'agrégation reste à l'appelant :** les corps divergent (retour `None` / `list[str]` union / `dict` de compteurs) → un helper qui agrège lui-même devrait connaître chaque mode de réduction. Il retourne donc les résultats des tenants traités **sans erreur** (un tenant qui lève ne contribue aucun élément) et laisse l'appelant réduire. **Préservation exacte du partiel-échec** : pour `price`/`composite` (où l'original appendait au fil de l'eau dans une liste partagée, un tenant échouant en cours gardant les tickers déjà ajoutés), le corps mute une **liste de fermeture** (effet de bord) → comportement identique ; pour `screener`/`retention` (agrégation atomique post-boucle), le corps **retourne** un tuple réduit après la boucle. Équivalence vérifiée en revue indépendante.

**Livrables :**
- `app/workers/tenant_iteration.py` (nouveau) — `for_each_tenant(db_pool, body, *, error_message) -> list[T]` : exécute le `SELECT id FROM tenants ORDER BY created_at`, boucle, pose `tenant_scope(tenant_id)`, appelle `await body(tenant_id)`, et sur exception logge `error_message` (format à un `%s` recevant l'UUID) + `continue` (best-effort par tenant). Docstring FR du WHY (énumérer-puis-scoper indissociable, tolérance par tenant). Émet un log de comptage uniforme (« Itération multi-tenant — N tenant(s) »), parité d'observabilité sur les 5 chemins (retention en gagne un qu'il n'avait pas).
- `app/workers/tasks.py` — les **5 hôtes** appellent `for_each_tenant(...)` au lieu de ré-implémenter la boucle ; chacun garde son message d'erreur bespoke et sa sémantique d'agrégation. Imports `PurgeResult` + `from uuid import UUID` ajoutés ; `tenant_scope` conservé (toujours utilisé par `_execute_usage_reporting` + `_execute_analysis`).
- `tests/workers/test_tenant_iteration.py` (nouveau) — 6 tests unitaires : N tenants → N résultats ordonnés ; corps exécuté sous le bon contexte (`get_current_tenant() == tenant_id`, non-vacuous) ; 0 tenant → liste vide (corps jamais appelé) ; **best-effort** (1 tenant lève → les autres traités, prouvé non-vacuous) ; erreur loggée via `error_message` + UUID ; contexte restauré après échec (pas de fuite ContextVar).

**Preuve d'acceptation observable** : `grep -c "SELECT id FROM tenants ORDER BY created_at" app/workers/tasks.py` == **0** (le helper détient l'unique copie) ; les 5 hôtes appellent `for_each_tenant(...)` ; les 5 tests workers existants (price_alert, composite_alert, scheduled_screener, retention_purge, watchlist_analysis) restent **verts sans re-pointer un seul mock** (le helper exécute le même `SELECT` sur le même `db_pool` → les mocks `pool.fetch` interceptent inchangés). **Pas de Docker/PG dans le conteneur web** → preuve par grep + tests unitaires (faux pool). **Pas d'eval** (plomberie workers — aucun prompt de skill ni l'orchestrateur de skills touché).

**Version** : 10.75.0
**Tests** : 2 363 backend collectés (2 321 passés, 41 skipped, 1 xfailed — **+6 CI** : helper `for_each_tenant` [N résultats ordonnés ; contexte par tenant non-vacuous ; liste vide ; best-effort isolation ; format `error_message`+UUID ; restauration contexte]) ; `ruff` + `mypy app/ --ignore-missing-imports` (171 fichiers, +1) verts ; **frontend non touché** (Vitest inchangés par construction). **Revue indépendante à contexte frais (sous-agent dédié, nourri des critères d'acceptation)** : **correctness** (effort high) — **0 BLOCKER/MAJOR** : 5 critères MET (types de retour préservés ; messages d'erreur verbatim ; agrégation préservée ; best-effort non-vacuous ; 5 tests verts sans re-pointage), équivalence partiel-échec vérifiée (mutation de fermeture ≡ append au fil de l'eau pour price/composite ; retour-puis-réduit ≡ agrégation post-scope pour screener/retention), réconciliation confirmée (`usage_reporting`/`_execute_analysis` correctement non touchés), lambda non-async retournant une coroutine correctement awaitable, pas de fuite de contexte (`tenant_scope` `finally: reset`), zéro import/variable orphelin ; **1 MINOR observable** (logs INFO de comptage pré-boucle supprimés) **résolu** (log de comptage uniforme ajouté au helper → parité rétablie + retention en gagne un) ; MINOR de style écarté (sans objet). **Qualité** (`/simplify` 4 axes) : aucun changement — le refactor EST la simplification (5 copies → 1) ; **écarté** : uniformiser price/composite en style retour (la mutation de fermeture préserve le partiel-échec exact — la cohérence stylistique ne vaut pas un changement de sémantique).

### Sprint 187 — Refactor : helper `create_runtime_pool()` (invariant DSN runtime + setup RLS) ✅

**Objectif :** Rendre **impossible** de créer un pool asyncpg runtime qui résout la bonne DSN (`resolve_app_database_url()` → rôle `app_runtime`, NOSUPERUSER/NOBYPASSRLS) mais oublie le hook de contexte tenant (`setup=apply_tenant_context`), ou l'inverse — oublier l'un OU l'autre rend l'isolation RLS multi-tenant **silencieusement** inerte. Finding d'altitude répété des revues S182 ET S185. Sprint **backend seul** (aucun fichier `frontend/` touché → non-régression front par construction).

**Décision tranchée — home `app/db/pool.py` (nouveau fichier) plutôt que co-localisation dans `tenant_context.py` :** le helper compose **deux leaves de couches différentes** — `apply_tenant_context` (`app/db/tenant_context.py`, primitive RLS par connexion : ContextVar + GUC, n'importe que `asyncpg` + `app.models.tenant`) et `resolve_app_database_url` (`app/utils/security_config.py`, politique DSN/rôle). Co-localiser dans `tenant_context.py` y traînerait l'import de `security_config` (env, fail-closed) → inversion de couche : une primitive RLS se mettrait à raisonner sur *où* vit la DB. `pool.py` est précisément le **seam de composition** qui joint les deux leaves (import propre vers le bas, aucun cycle). `app/db/` = package de composition DB (`tenant_context.py` primitives, `provision_app_runtime.py` setup de rôle au boot, `pool.py` construction de pool).

**Décision tranchée — l'invariant reste à DEUX décisions (DSN + setup), `require_secure_db_url()` NON absorbé dans le helper :** le garde insecure-creds vit uniquement à `app/api/main.py` (boot API). L'absorber dans le helper donnerait aux **9 sites workers** un garde fail-closed qu'ils n'ont pas aujourd'hui → **changement de comportement**, contraire à la contrainte « zéro changement runtime » du sprint. C'est de plus une assertion de boot (une fois) vs un constructeur appelé ~10×. Généralisation **différée** en sprint suggéré (clore le gap uniformément, avec tests de boot workers).

**Livrables :**
- `app/db/pool.py` (nouveau) — `create_runtime_pool(*, min_size: int, max_size: int) -> asyncpg.Pool` : résout `resolve_app_database_url()` (DSN positionnelle) ET câble `setup=apply_tenant_context` en interne ; l'appelant ne fournit que les tailles. Signature **keyword-only** (interdit un swap positionnel `(10, 2)`). Docstring FR du WHY (les deux décisions indissociables pour la RLS). DSN et `setup` **hardcodés** — non surchargeables, pour que le chokepoint ne fuie pas.
- `app/api/main.py` — site API (2/10) passe par le helper ; `import asyncpg` + `apply_tenant_context` retirés (devenus inutilisés) ; `require_secure_db_url(db_url)` **conservé** comme garde de boot.
- `app/workers/tasks.py` — les **9 sites** (1/3) passent par le helper ; `resolve_app_database_url` + `apply_tenant_context` retirés des imports ; `import asyncpg` **conservé** (toujours requis pour les annotations `asyncpg.Pool`).
- `tests/db/test_pool.py` (nouveau) — 2 tests unitaires : l'invariant verrouillé (DSN == `resolve_app_database_url()` ET `setup is apply_tenant_context`) + propagation des tailles workers (1/3).

**Réconciliation carte↔code (anti-hallucination) :** la carte présentait le « ripple » de ~25 mocks `patch(...asyncpg.create_pool...)` comme **le cœur du sprint** — prémisse **fausse**. Tous les sites utilisent `import asyncpg` (import de module), donc `app.workers.tasks.asyncpg` ET `app.db.pool.asyncpg` désignent **le même objet module** ; `patch("app.workers.tasks.asyncpg.create_pool")` comme `patch("asyncpg.create_pool")` mutent l'attribut `create_pool` du module asyncpg partagé → l'appel du helper reste intercepté. **0 mock re-pointé**, suite verte par construction (confirmé empiriquement).

**Preuve d'acceptation observable** : `grep -c "asyncpg.create_pool(" app/workers/tasks.py app/api/main.py` == **0** + **0** (tous les sites passent par le helper) ; un seul `create_runtime_pool(min_size=1, max_size=3)` suffit à obtenir un pool RLS-correct. **Pas de Docker/PG dans le conteneur web** → preuve par grep + tests unitaires (mock `asyncpg.create_pool`, assertion des kwargs). **Pas d'eval** (plomberie de pool — aucun prompt de skill ni l'orchestrateur de skills touché).

**Version** : 10.74.0
**Tests** : 2 357 backend collectés (2 315 passés, 41 skipped, 1 xfailed — **+2 CI** : `create_runtime_pool` câble DSN runtime + `setup=apply_tenant_context` [assertion des kwargs, non-vacuous] ; propage les tailles workers 1/3) ; `ruff` + `mypy app/ --ignore-missing-imports` (170 fichiers, +1) verts ; **frontend non touché** (Vitest inchangés par construction). **Revue indépendante à contexte frais (2 passes)** : **correctness** (`/code-review` high, sous-agent dédié nourri des critères d'acceptation) — **0 BLOCKER/MAJOR/MINOR** : 5 critères MET (helper câble les deux invariants ; 10 sites migrés, tailles préservées 2/10 API + 1/3 ×9 ; suite verte ; test unitaire non-vacuous ; preuve grep == 0), `require_secure_db_url` toujours exécuté au boot, aucun import/variable orphelin, raisonnement d'interception des patchs vérifié empiriquement (aucun test devenu no-op). **2ᵉ passe non relancée** (aucun octet de code corrigé en correctness). **Qualité** (`/simplify` 4 axes, 4 sous-agents) : **1 finding appliqué** (commentaire `main.py` trimé — il dupliquait la docstring du helper post-refactor ; recentré sur le fait propre au site : tailles + défaut legacy E3-S4) ; **différé** (noté en SPRINTS SUGGÉRÉS) : absorber `require_secure_db_url` dans le helper pour que tous les pools runtime (workers inclus) héritent du garde insecure-creds — vraie généralisation mais **changement de comportement** hors périmètre ; **écartés** : re-pointer les ~20 mocks workers vers le nouveau home (churn hors-diff, ils interceptent correctement via le module partagé ; `import asyncpg` non vestigial — requis par les annotations), double `resolve_app_database_url()` au boot API (coût négligeable — lecture env pure une fois au démarrage ; la redondance protège l'invariant), fusion des 2 tests du helper (le 2ᵉ documente le pattern 1/3 réel, cheap).

### Sprint 186 — Ops : durcissement du provisioning DB (non-propriété `app_runtime` + revoke `PUBLIC`) ✅

**Objectif :** Fermer le **résidu §2.3** de `docs/revue-owasp-rls-2026-06.md` que la migration `0011` (S182) avait **explicitement différé** (cf. son docstring) : garantir que `app_runtime` n'est propriétaire d'aucune table RLS (sinon il pourrait `ALTER … DISABLE/NO FORCE ROW LEVEL SECURITY` et contourner sa propre RLS) **et** révoquer les privilèges `PUBLIC` par défaut sur le schéma (sans quoi tout rôle de connexion, présent ou futur, hérite d'un accès sans GRANT explicite). Sprint **backend/infra seul** (migration Alembic + 1 ligne CI ; aucun fichier `frontend/` ni `app/` applicatif touché → non-régression par construction).

**Décision tranchée — (a) migration `0012` (revoke `PUBLIC` + re-GRANT) plutôt que (b) test d'assertion seul :** le revoke `PUBLIC` est un **durcissement réel** exigé par la revue OWASP §2.3 — un test seul ne corrige rien. **Subtilité vérifiée et documentée** : les GRANTs de `0011` sont accordés **directement** à `app_runtime`, donc `REVOKE … FROM PUBLIC` ne les touche pas — le runtime conserve son accès **même sans** re-GRANT. Le re-GRANT (même périmètre que `0011`, **verrouillé par test anti-divergence**) rend `0012` auto-portant comme source d'accès explicite ; le durcissement effectif est le revoke des privilèges `PUBLIC` par défaut sur le schéma (`USAGE`/`CREATE`). **Non-propriété assertée, jamais transférée** : les tables restent la propriété de `copilote` pour qu'Alembic puisse migrer — on ne transfère JAMAIS la propriété à `app_runtime`.

**Livrables :**
- `alembic/versions/0012_revoke_public_privileges.py` (`down_revision = "0011_app_runtime_role"`, head unique) — `_REVOKE_PUBLIC` (`REVOKE ALL ON SCHEMA public FROM PUBLIC` + `… ON ALL TABLES …` + `… ON ALL SEQUENCES … FROM PUBLIC`) puis `_REGRANT` (miroir mot pour mot de `0011._GRANTS` : USAGE schéma, SELECT/INSERT/UPDATE/DELETE sur les 11 tables RW, SELECT `plan_limits`, USAGE séquences). Idempotente (`REVOKE`/`GRANT` natifs). `downgrade()` re-`GRANT ALL ON SCHEMA public TO PUBLIC` (état Postgres par défaut) **sans** retirer les GRANTs directs de `0011`. Aucun mot de passe (hygiène secrets, cohérent S182).
- `tests/test_alembic_revoke_public_privileges.py` — forme de migration sans DB : chaînage après `0011`, présence des `REVOKE … FROM PUBLIC`, re-GRANT au seul `app_runtime` (aucun `TO PUBLIC` à l'upgrade), **verrou anti-divergence** (`0012._RW_TABLES == 0011._RW_TABLES` et `set(_REGRANT) == set(0011._GRANTS)`), downgrade restaure `PUBLIC`, pas de `;` terminal.
- `tests/integration/test_revoke_public_rls.py` (`@pytest.mark.integration`, skippé hors PG migré, ajouté au gate NOSUPERUSER de `.github/workflows/ci.yml`) — sous le rôle réel `app_runtime` : non-propriété des 7 tables RLS (`pg_class.relowner ≠ app_runtime`), **zéro privilège via `PUBLIC`** (inspection `pg_namespace.nspacl` + `pg_class.relacl` — aclitem à grantee vide), et accès conservé via le GRANT explicite (`has_table_privilege`/`has_schema_privilege`).

**Preuve d'acceptation observable** : après `alembic upgrade head`, `app_runtime` a ses accès via GRANT explicite et **zéro via `PUBLIC`** ; un rôle anonyme/`PUBLIC` ne voit plus le schéma. **Pas de Docker/PG dans le conteneur web** → les 4 tests d'intégration y sont skippés (dit explicitement) et la preuve tourne en CI sous le rôle réel ; la forme de migration (chaînage, revoke, re-GRANT, non-divergence) est prouvée en CI standard sans PG. `alembic heads` confirme `0012` head unique. **Pas d'eval** (infra DB pure — aucun prompt de skill ni l'orchestrateur de skills touché).

**Version** : 10.73.0
**Tests** : 2 355 backend collectés (2 313 passés, 41 skipped [+4 : revoke `PUBLIC` RLS sous rôle réel, skippés hors PG migré], 1 xfailed — **+12 CI** : 8 forme migration [chaînage `0012`→`0011` ; `REVOKE … FROM PUBLIC` schéma/tables/séquences ; re-GRANT au seul `app_runtime` sans `TO PUBLIC` ; non-divergence `_RW_TABLES`/`_REGRANT` vs `0011` ; pas de mot de passe ; downgrade restaure `PUBLIC` ; pas de `;` terminal ; upgrade/downgrade appelables] + 4 intégration RLS skippées [non-propriété `pg_class.relowner` ; zéro `PUBLIC` sur schéma `nspacl` ; zéro `PUBLIC` sur tables `relacl` ; accès conservé via GRANT explicite]) ; `ruff` + `mypy app/ --ignore-missing-imports` (169 fichiers) verts ; **frontend non touché** (Vitest inchangés par construction).
...
### Sprint 181 — E5-S4 : metering du screener planifié + alertes composites ✅

**Objectif :** Fermer le dernier reliquat de consommation planifiée non facturée. Après S177, seule la re-analyse watchlist hebdomadaire (`run_watchlist_analysis`) tournait sous le tenant propriétaire et était métrée ; le **screener planifié** (`run_scheduled_screener`, dimanche 11h UTC) et les **alertes composites** (`run_composite_alert_check`) lisaient encore la watchlist sous le tenant legacy avec un orchestrateur non metré → leur conso n'était imputée à aucun tenant et échappait à `run_usage_reporting` (S174). Sprint **backend/worker seul** : threading tenant + metering des deux chemins, sans réécrire le cœur de `CompositeAlertService` ni `ScreenerService` (ils lisent la watchlist via la RLS-GUC, donc une simple itération sous `tenant_scope` suffit).

**Décision tranchée — agrégation union (screener) :** le webhook FORT, le rapport PDF et le résumé Slack sont **agrégés sur l'union des tenants** (un seul envoi par run — comportement observable équivalent au run legacy unique), tandis que la lecture watchlist, le metering et l'écriture `alert_history` (RLS) restent **par tenant sous `tenant_scope`**. Alternative écartée : émettre un webhook par tenant (multiplie les notifications, diverge du comportement observable actuel).

**Livrables :**
- `app/workers/tasks.py` — `_execute_composite_alert_check` et `_execute_scheduled_screener` itèrent `SELECT id FROM tenants ORDER BY created_at` (pool hors-RLS) puis, sous `tenant_scope(tenant_id)`, exécutent le travail avec `_build_orchestrator(with_metering=True)`. Best-effort par tenant (try/except, ContextVar restauré par `tenant_scope`). Helper `_screen_tenant_watchlist` extrait (lecture watchlist RLS + screening métré + `alert_history` + alertes ESG du tenant courant ; retourne `(nb_tickers, tickers_fort, screen_entries)` pour l'agrégation union). Fuite de pool corrigée sur le chemin composite (build de l'orchestrateur déplacé dans le `try`, garde `None` au `finally` — parité avec le chemin screener).
- `.github/workflows/ci.yml` — `tests/integration/test_scheduled_metering_rls.py` ajouté au gate RLS NOSUPERUSER.

**Preuve d'acceptation observable** : après un run planifié, la conso d'un screener / d'une vérif d'alerte composite du tenant B apparaît dans `usage_events` imputée à B, jamais au tenant legacy. Prouvé sans PG par capture de `get_current_tenant()` au site screen()/`check_composite_alerts()` (== tenant propriétaire, non-vacuous via `_TENANT_A/B != LEGACY`) ; prouvé **bout-en-bout RLS** (skippé hors PG migré, exécuté en CI) par `test_scheduled_metering_rls.py` — entrée watchlist écrite sous B, run planifié → `usage_event` visible sous B, **masqué sous legacy**. **Pas de Docker/PG dans le conteneur web** → les 2 tests bout-en-bout y sont skippés (dit explicitement). **Pas d'eval** (aucun prompt de skill ni l'orchestrateur de skills touché — worker + threading tenant uniquement).

**Périmètre — différé :** `run_full_analysis` (déclenché par alerte prix via `.delay()` Celery) reste sous legacy — le passage de tenant à travers la frontière du broker est un sujet distinct (le ContextVar ne traverse pas la sérialisation Celery ; S185 suggéré).

**Version** : 10.68.0
**Tests** : 2 306 backend collectés (2 271 passés, 34 skipped [+2 : metering screener/composite RLS, skippés hors PG migré], 1 xfailed — **+3 CI** : composite [orchestrateur métré ; chaque tenant sous son `tenant_scope` capturé non-vacuous ; best-effort un tenant en échec n'interrompt pas les autres + ContextVar restauré] ; screener [orchestrateur métré ; capture scope par tenant ; lecture watchlist scopée — B vide non screené ; webhook FORT agrégé sur l'union ; best-effort + ContextVar restauré ; erreur de screen ignorée] ; **+2 intégration RLS** skippées : screener + alerte composite imputés à B, masqués sous legacy) ; `ruff` + `mypy app/ --ignore-missing-imports` (166 fichiers) verts ; **frontend non touché**. **Revue indépendante à contexte frais** : **correctness** (`/code-review` high, sous-agent dédié nourri des critères d'acceptation) — **0 BLOCKER/MAJOR** ; 5 critères d'acceptation MET (énumération par tenant + orchestrateur métré, best-effort + ContextVar restauré, threading composite, agrégation union screener, propagation ContextVar→GUC sur les deux pools y compris à travers l'`asyncio.gather` du screener et de `_emit_usage_events`) ; 1 **MINOR corrigé** (fuite `db_pool` sur chemin d'erreur composite si `_build_orchestrator` lève → build déplacé dans le `try` + garde `None`, miroir du chemin screener) + 1 NIT écarté (`cout_total_usd=0.0` pré-existant, hors périmètre) ; **2ᵉ passe RESOLVED** (correctif mécaniquement identique au chemin screener déjà validé, tests verts). **Qualité** (`/simplify` 4 axes) : **1 finding appliqué** (contrat de `_screen_tenant_watchlist` resserré — retourne `list[str]` de tickers FORT au lieu de `list[Any]` d'entrées) ; écartés : extraction d'un `for_each_tenant(...)` (altitude — idiome de 3 lignes, corps trop dissemblables, le bon seam `_screen_tenant_watchlist` est déjà extrait), `tests/workers/conftest.py` partagé (convention par-fichier établie dans `tests/workers/`, hors périmètre), hoist de `WatchlistService` (coût allocation seul, helper auto-contenu plus lisible).
...
## Blocs d'avertissement historiques — déplacés de `ROADMAP.md` (rotation 2026-06-12)

> Contexte des sprints 125-149, déjà archivés ci-dessus ; conservés verbatim.
