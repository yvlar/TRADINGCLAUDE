...
- **Correctifs post-revue PR** : (#2) `report.py::_reconstruct_response` ne reconstruisait que la traçabilité Graham → les six champs sont désormais reconstruits, alignant le PDF rejoué depuis l'historique sur `/ticker-report`. (#2/#3) extraction `input_data` factorisée dans un module partagé `app/api/endpoints/ratios_recon.py` (`reconstruct_ratios_traces`), consommé par les deux endpoints ; le triplet `_*_ratios_trace` répété aux 4 sites de `core.py` est factorisé dans `_request_ratios_traces(request)`. (#4) tests ajoutés : reconstruction earnings/valuation côté `/report`, helper `_request_ratios_traces` (six champs, branche source-sans-date).
...
**Version** : 10.32.0
**Tests** : 1 707 backend collectés (1 703 passés, 3 skipped, 1 xfailed — +13) ; `ruff All checks passed` ; 431 Vitest verts (+5) ; `tsc --noEmit` 0 erreur ; ESLint 0/0.
