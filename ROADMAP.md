...
- `frontend/src/components/AnalysisResult.tsx` — helper présentationnel `RatiosSourceNote` (gate `if (!fetchedAt) return null` — honnêteté None), rendu sous la carte earnings (`data-testid="earnings-ratios-source"`) et sous la carte valuation (`data-testid="valuation-ratios-source"`), à l'intérieur du gate de présence de la carte. Affichage Graham (Sprint 139) inchangé.
...
**Version** : 10.32.0
**Tests** : 1 702 backend collectés (1 698 passés, 3 skipped, 1 xfailed — +8) ; `ruff All checks passed` ; 430 Vitest verts (+4) ; `tsc --noEmit` 0 erreur ; ESLint 0/0.
