...
**Dernière mise à jour : 2026-05-31 — Sprint 142 complété**
...
| **Sprint actif** | Sprint 145 — Provenance par ratio sur l'analyse rendue (AnalysisResult) |
...
- `POST /analyze` — 16 skills tier2 + cache Redis + cache composite_score < 24h (Sprint 65 — circuit court DB) ; **scores financiers déterministes** (Altman Z, Beneish M, Piotroski F, Montier C, Sloan, Nombre de Graham) calculés en Python (`app/services/financial_calculations.py`) et substitués au bloc LLM — le modèle interprète, il ne produit plus les chiffres (Sprint 128) ; **sous-composantes auditables** (8 indices Beneish DSRI/GMI/… + termes X1-X5 Altman) également calculées en Python et persistées dans l'output — analyse entièrement rejouable (Sprint 131) ; **signaux détaillés F/C déterministes** — les 9 critères Piotroski (`f_score.criteria[].passe`) et 6 signaux Montier (`c_score.signaux[].present`) calculés en Python par signal, cohérents par construction avec le score agrégé (`sum(passe)==f_score`, `sum(present)==c_score`) ; donnée manquante/financière → sortie LLM conservée, jamais un `False` trompeur (Sprint 142) ; **ossature DCF déterministe** (`stock_valuation`) — WACC (CMPC), valeur intrinsèque DCF par action et matrice de sensibilité WACC×g calculées en Python (`app/services/valuation_calculations.py`) et substituées au bloc LLM ; le modèle conserve comparables, sectoriel et verdict ; financières/REIT exclues du DCF (méthode sectorielle prime) (Sprint 132)
...
**Objectif :** Seuls les **scores agrégés** F-Score (0-9) et C-Score (0-6) étaient déterministes (Python + substitution post-parse, Sprints 128/131) ; les **signaux individuels** — `f_score.criteria[].passe` (9 critères Piotroski) et `c_score.signaux[].present` (6 signaux Montier) — restaient **interprétés par le LLM**, donc non rejouables et susceptibles de diverger du score agrégé qu'ils sont censés composer. Calculer ces booléens en Python et les substituer, en cohérence stricte avec le score agrégé. Suite de la file revue expert FinTech. **Sprint backend pur** (aucun frontend, aucune migration).

**Livrables :**
- `app/services/financial_calculations.py` — nouvelles fonctions pures par signal `piotroski_f_score_detail(...) -> PiotroskiComponents` et `montier_c_score_detail(...) -> MontierComponents` (dataclasses `CritereBinaire(nom, valeur, detail)` + conteneurs `criteres`/`signaux` + score agrégé). Le score est désormais `sum(1 for c in criteres if c.valeur)` → **invariant par construction** `sum(passe)==f_score` / `sum(present)==c_score`. Les anciennes `piotroski_f_score`/`montier_c_score` deviennent des **wrappers minces** déléguant à `.f_score`/`.c_score` (signatures inchangées → aucun appelant cassé). Chaque booléen reproduit **exactement** la condition `score += 1` d'origine (zéro dérive vérifiée par revue indépendante). Helper `_detail_comparatif` factorisant le détail « T vs T-1 / donnée manquante »
- `app/skills/tier2/earnings_quality/skill.py` — `_ScoresDeterministes` porte désormais `f: PiotroskiComponents` / `c: MontierComponents` (au lieu des entiers) ; `_injecter_scores` écrase `data["f_score"]["criteria"]` et `data["c_score"]["signaux"]` par les listes Python **seulement si le calcul a abouti** (listes non vides). Donnée manquante / financière (Piotroski exclut les financières) → sortie LLM conservée, jamais un `False` trompeur — exactement comme les entiers `f/c_score` (skill.py)
- **Décision de périmètre** : limité aux signaux F/C. M-Score (8 indices) et Z-Score (X1-X5) déjà déterministes (Sprint 131), non touchés. C-Score (Montier) n'a **pas** de gate sectoriel `is_financial` (comportement d'origine conservé) ; seul F (Piotroski) l'a
- Tests : unitaires `financial_calculations` (9 critères / 6 signaux + **invariant** `sum==score` + cas donnée manquante → booléen `False` avec détail explicite + banque → listes vides/score None) ; intégration `_injecter_scores` (substitution des listes + invariant ; financière → critères F LLM conservés ; listes Python vides → blocs LLM conservés) ; bout en bout `execute` (la sortie validée porte les booléens Python, pas ceux du LLM)

**Version** : 10.28.0
**Tests** : 1 667 backend collectés (1 663 passés, 3 skipped, 1 xfailed — +12) ; ruff `All checks passed` ; frontend inchangé (sprint backend pur → Vitest/tsc/ESLint non concernés)

**Note d'environnement :** session web — la substitution modifie l'output `earnings_quality` → **les evals ciblées (Claude réel) devraient être relancées, mais `ANTHROPIC_API_KEY` est absente du conteneur web → non exécutées ici, à faire en local** (cf. Sprint 137 différé). Canal d'exécution vérifié (sortie des commandes rendue). Réconciliation carte↔code : toutes les prémisses du chemin critique vérifiées par lecture avant implémentation (`FScoreCriterion` `schemas.py:132` / `CScoreSignal` `:144` ; `_injecter_scores` `skill.py:154` substituait déjà les entiers f/c, PAS les listes ; `piotroski_f_score`/`montier_c_score` retournaient des entiers `financial_calculations.py`). Preuve d'acceptation constatée : sur les ratios MSFT, F=4/C=0 avec `sum(passe)==f_score` et `sum(present)==c_score`, détails honnêtes sur les données manquantes. Revue indépendante à contexte frais (sous-agent correctness `high`, distinct de la session auteur, nourri des critères d'acceptation) : **aucun bug HIGH/MED** ; les 15 booléens confirmés identiques aux conditions agrégées d'origine (zéro dérive), None-safety des f-strings vérifiée. Passe qualité `/simplify` (sous-agent) : 1 cleanup appliqué (helper `_detail_comparatif` consolidant le pattern de détail répété 6×) ; 2 non-findings écartés à raison (wrappers parallèles au Sprint 131 ; les 2 list-comprehensions utilisent des clés de schéma distinctes `passe`/`present`). Stack Docker non démarrée. Pas de test navigateur live.
